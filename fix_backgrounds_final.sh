...
echo "í¼‘ Fixing Dark Mode Backgrounds & Button Visibility..."

# =========================================================
# 1. FIX HOME PAGE BACKGROUND (Remove Hardcoded White)
# =========================================================
echo "í¿  Cleaning Home Page Container..."
...
export default function Home() {
  ...
  if (!isClient) return <div className="min-h-screen" />; // Removed bg-[#F8FAFC]
  ...
  return (
    <div className="w-full px-6 space-y-10 pt-4 pb-20">
      ...
      {activeCategory === "All" && !searchQuery && favorites.length > 0 && (
        <section className="animate-in fade-in slide-in-from-top-4">
            <div className="flex items-center gap-2 mb-4 px-1 opacity-90">
                ...
                <h2 className="text-xs font-bold text-muted dark:text-slate-400 uppercase tracking-wider">Favorites</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                {favoriteTools.map(tool => (
                    <Link key={tool.id} href={tool.href} className="group flex items-center gap-4 p-5 bg-surface dark:bg-slate-800/50 border border-line dark:border-slate-700 hover:border-[rgb(117,163,163)]/50 transition-all duration-200 hover:-translate-y-0.5 relative">
                        ...
                        <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-main dark:text-slate-100 text-sm truncate group-hover:text-[rgb(117,163,163)] transition-colors">{tool.name}</h4>
                            <p className="text-xs text-muted dark:text-slate-400 truncate mt-1">{tool.desc}</p>
                        </div>
                        <button onClick={(e) => toggleFavorite(e, tool.id)} className="absolute top-4 right-4 p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition text-amber-400 opacity-0 group-hover:opacity-100">
      ...
      <section>
        <div className="flex items-center gap-2 mb-4 px-1 opacity-90">
          <LayoutGrid size={18} className="text-muted/70 dark:text-slate-500"/>
          <h2 className="text-xs font-bold text-muted dark:text-slate-400 uppercase tracking-wider">
          ...
          {filteredTools.length > 0 ? (
            filteredTools.map((tool) => (
              <Link 
                key={tool.id} 
                href={tool.href}
                className={`flex items-center gap-4 p-5 bg-surface dark:bg-slate-800/50 rounded-2xl border border-line dark:border-slate-700 hover:border-[rgb(117,163,163)]/50 transition-all duration-200 group hover:-translate-y-0.5 relative ${tool.status === 'Soon' ? 'opacity-60 grayscale cursor-not-allowed' : ''}`}
              >
            ...
            <div className="col-span-full py-20 text-center text-muted/70 dark:text-slate-500 border-2 border-dashed border-line dark:border-slate-800 rounded-2xl bg-background dark:bg-slate-900/50">
...
# =========================================================
# 2. FIX BUTTON VISIBILITY (Dark Mode Support)
# =========================================================
echo "í´˜ Updating Button Component for Dark Mode..."
...
export default function Button({ 
  ...
  const variants = {
    ...
    // Secondary: Outline / Card Surface
    ...
    // Ghost: Transparent but visible on hover
...
echo "âœ… Backgrounds & Buttons Fixed. Run 'npm run dev'!"
