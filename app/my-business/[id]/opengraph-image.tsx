...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  // a mislabeled "My Business" image for a URL that 404s.
  const tool = rawTool && rawTool.category === 'My Business' ? rawTool : null;
  ...
  return new ImageResponse(
    <OGImageTemplate eyebrow="My Business" title={title} subtitle={subtitle} />,
    { ...size }
  );
}
