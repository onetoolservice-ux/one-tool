    ...
    <div className="fixed inset-0 z-[9999] bg-slate-950/60 backdrop-blur-sm flex items-start justify-center pt-[15vh] px-4 animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center border-b border-slate-100 dark:border-slate-800 px-4 py-4">
          <Search className="w-5 h-5 text-slate-600 dark:text-slate-300 mr-3" />
          <input 
            autoFocus
            className="flex-1 bg-transparent outline-none text-lg text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400"
            placeholder="Type a command or search..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          <div className="text-xs text-slate-600 dark:text-slate-300 font-medium bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">ESC</div>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 scrollbar-hide">
            ...
            <div className="mb-2">
              ...
              >
                ...
                <div className="flex-1">
                   ...
                   <div className="text-xs text-slate-600 dark:text-slate-400 line-clamp-1 font-mono mt-0.5">
          ...
          {filteredTools.length === 0 ? (
            <div className="py-12 text-center text-slate-600 dark:text-slate-400 text-sm">No results found.</div>
          ) : (
            <div className="space-y-1">
              <div className="px-2 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Tools</div>
              {filteredTools.map((tool) => (
                ...
                >
                  <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded-md text-slate-600 dark:text-slate-300 group-hover:text-indigo-500 group-hover:bg-white dark:group-hover:bg-slate-700 transition-colors">
                    {renderIcon(tool.icon)}
                  </div>
                  <div className="flex-1">
                     <div className="font-medium">{tool.name || (tool as any).title || "Unnamed Tool"}</div>
                     <div className="text-xs text-slate-600 dark:text-slate-300 line-clamp-1">{tool.desc}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-950/50 px-4 py-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-600 dark:text-slate-300">
