  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-6">
        ...
        {/* Custom date range */}
        {range === 'custom' && (
          <div className="flex items-center gap-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-4">
        ...
        {isEmpty ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { label: 'Total Income', value: pl.income },
                { label: 'Total Expenses', value: pl.expenses },
                { label: 'Net Profit', value: pl.gross },
              ].map(card => (
                <div key={card.label} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">{card.label}</p>
                  <p className="text-3xl font-semibold text-neutral-value">{fmtCurrency(card.value)}</p>
                        ...
                        return (
                          <div key={cat}>
                            <div className="flex justify-between text-xs mb-1">
                              ...
                              <span className="font-black text-neutral-value">{fmtCurrency(val)}</span>
                            </div>
            ...
            {pendingInvoices.length > 0 && (
              <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-5">
                ...
                <div className="flex items-center gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                  <div className="flex-1">
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Expected Incoming</p>
                    <p className="text-2xl font-semibold text-neutral-value">{fmtCurrency(forecastTotal)}</p>
                  </div>
                    ...
                    {pendingInvoices.slice(0, 4).map(inv => {
                      ...
                      return (
                        <div key={inv.id} className="flex items-center gap-2 text-xs">
                          ...
                          <span className="font-semibold text-neutral-value">{fmtCurrency(inv.total)}</span>
