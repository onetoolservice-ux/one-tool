...
// ═══════════════════════════════════════════════════════════════════════════════
// EXPENSES — Debit transaction list, SAP Fiori-style VH filters
// ═══════════════════════════════════════════════════════════════════════════════
...
export function Expenses() {
  ...
  const filtered = useMemo(() => {
    // Expenses = debit transactions only
    let txns = allTxns.filter(t => t.type === 'debit');
  ...
  const kpi = useMemo(() => {
    ...
    return { totalSpend, count: filtered.length, selfTransferCount };
  }, [filtered]);
  ...
  const handleExport = () => {
    ...
    const headers  = ['Date', 'Description', 'Category', 'Amount', 'Transfer', 'Loan', 'Recurring', 'Account', ...dynKeys];
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl shadow-2xl w-80 p-5 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowBulkCatDialog(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                <X size={15} />
              </button>
            </div>
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        {showFilterBar && (
          <div className="px-4 py-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-x-4 gap-y-3">
            ...
            {/* Name — inline quick search + VH icon */}
      ...
      <div className="bg-white dark:bg-slate-900 overflow-hidden">
        ...
        <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center gap-3 min-h-[44px]">
          ...
          {hasData && (
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <PFButton variant="danger" icon={<Trash2 size={14} />} onClick={handleBulkDelete}
                disabled={selectedIds.size === 0}
                title={selectedIds.size > 0 ? `Delete ${selectedIds.size} selected` : 'Select rows to delete'}>
                Delete
              </PFButton>

              <PFButton variant="primary" icon={<FileDown size={14} />} onClick={handleExport}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        <input type="checkbox" checked={isSelected} onChange={() => toggleRow(txn.id)} className="rounded cursor-pointer" />
                      </td>
                      ...
                      <td className="px-4 py-2.5">
                        {editingCat ? (
                          <select autoFocus defaultValue={txn.category}
                            onChange={e => {
        ...
        {hasData && (
          <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-400 dark:text-slate-600">
            Click a <span className="font-medium text-slate-500 dark:text-slate-400">category</span> to reassign it, or a <span className="font-medium text-slate-500 dark:text-slate-400">description</span> to rename it.
            Select rows to use bulk actions. Click the <span className="font-medium text-slate-500 dark:text-slate-400">Copy icon</span> on any filter for Value Help (SAP F4).
          </div>
        )}
      </div>
    </div>
