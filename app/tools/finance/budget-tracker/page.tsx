              ...
              {displayed.map(row => {
                ...
                return (
                  <Row key={row.id}>
                    ...
                    {columns.find(c => c.key === "status")?.visible && <td className="p-3 w-28"><div className={`inline-block px-2 py-1 text-xs rounded-full ${row.status === "Posted" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-main"}`}>{row.status}</div></td>}
