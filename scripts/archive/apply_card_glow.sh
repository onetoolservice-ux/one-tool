...
echo "í¾¨ Upgrading Tool Cards with Category Glows & Bold Text..."
...
# 2. RE-APPLY TOOL TILE WITH NEW GLOW & BOLD TEXT
# Using standard cat to avoid template literal issues, then using sed to fix the backslashes if needed? 
# No, we'll use the Python trick again because it was 100% reliable for special characters.
# We need to make the text BOLD and the shadow COLORFUL.

cat > apply_tile.py << 'PYTHON_EOF'
import os
    ...
    >
      ...
      `}>
        ...
        <div className="flex items-start justify-between mb-5">
          {/* Icon Box - Slightly larger and more padding */}
          <div className={`
        ...
        <div className="space-y-2 flex-grow">
          ...
          {/* Description: Clearer, slightly larger than before */}
...
with open("app/shared/ToolTile.tsx", "w", encoding="utf-8") as f:
    f.write(content)
    
print("âœ… ToolTile.tsx updated with colored shadows and bold text.")
PYTHON_EOF

python3 apply_tile.py
rm apply_tile.py

echo "âœ… Applied 'Smart Glow' logic."
echo "í±‰ Run 'npm run dev' and hover over a Finance card vs a Developer card."
