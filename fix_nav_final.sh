...
echo "🧟 Removing Zombie Folders & Fixing Build..."

# 1. DELETE ACCIDENTAL LITERAL FOLDERS
# These are causing the "reading 'charAt' of undefined" error because they are empty/wrong.
rm -rf app/tools/category
rm -rf app/tools/converters/page.tsx # If this exists as a folder/page conflict
rm -rf app/tools/design/page.tsx
rm -rf app/tools/developer/page.tsx
rm -rf app/tools/health/page.tsx
rm -rf app/tools/finance/page.tsx
rm -rf app/tools/documents/page.tsx
rm -rf app/tools/productivity/page.tsx

# 2. ENSURE DYNAMIC ROUTE IS ROBUST
# We update the dynamic category page to be extra safe during build time.
cat > app/tools/[category]/page.tsx << 'EOF'
"use client";
...
echo "✅ Zombie Folders Deleted & Build Logic Fixed."