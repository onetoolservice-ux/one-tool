...
console.log("í³‰ Optimizing Grid Sizing for 4-Column Layout...");

// 1. FIX DASHBOARD GRID (app/dashboard/page.tsx)
const dashboardContent = `"use client";

import Link from "next/link";
import { useMemo, useState, useEffect } from "react";
import { Sparkles, Search, Clock, Layers, Star, ChevronRight, ArrowRight, Home as HomeIcon } from "lucide-react";
...
export default function Dashboard() {
  ...
  // OPTIMIZED GRID: minmax(240px) ensures 4 cards fit on standard screens
  const gridClass = "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5";
        ...
        <div className="max-w-[1600px] mx-auto px-6 py-4">
           <div className="flex flex-col md:flex-row justify-between items-center gap-4">
             <div className="flex items-center gap-4">
                ...
                <Link href="/" className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400 transition-colors">
                   <HomeIcon size={16} />
                   <span>Home</span>
                </Link>
             </div>
                  ...
                  <div className="space-y-8">
                    {Object.entries(cat.subcategories).map(([subName, tools]) => (
                       <div key={subName}>
                          ...
                          {/* EXPLICIT 4-COLUMN GRID for Desktop */}
...
const categoryContent = `import Link from "next/link";
      ...
      <div className="w-full px-6 space-y-12 pt-10 pb-24 max-w-[1600px] mx-auto">
        {sortedSubcats.map((subName) => (
          <section key={subName}>
             ...
             {/* EXPLICIT 4/5 COLUMN GRID HERE TOO */}
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
...
fs.writeFileSync(path.join(process.cwd(), 'app', 'dashboard', 'page.tsx'), dashboardContent, 'utf8');
fs.writeFileSync(path.join(process.cwd(), 'app', 'tools', '[category]', 'page.tsx'), categoryContent, 'utf8');
console.log("âœ… Grid Sizing Optimized (Fixed to 4-5 cols).");
